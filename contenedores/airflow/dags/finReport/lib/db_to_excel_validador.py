...
class MultiTableToExcelExporter:
    ...
    def exportar(
        ...
        schema = "validador"
        tabla = f"{nombre_excel.lower()}_texto"

        sql_text = f"""
            SELECT substring(registro, 16, 8)
            FROM {schema}.{tabla} WHERE linea = 1
        """
        
        cur_tmp.execute(sql_text)

        row = cur_tmp.fetchone()
        
        if row is None or row[0] is None:
            self.logger.warning(
                f"No se encontró header (linea = 1) en {schema}.{tabla}. "
                f"El archivo '{nombre_excel}' no fue cargado. Se omite exportación."
            )
            cur_tmp.close()
            conn_tmp.close()
            return 
